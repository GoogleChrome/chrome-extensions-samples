...
// The action API does not expose a way to read the action's current
// enabled/disabled state, so we have to track it ourselves. Relevant feature
// request: https://bugs.chromium.org/p/chromium/issues/detail?id=1189295
let actionEnabled = true;
let showToggleState = document.getElementById('show-toggle-state');
document.getElementById('toggle-state-button')
...
document.getElementById('popup-options')
...
// If a popup is specified, our on click handler won't be called. We declare it
// here rather than in the `onclicked-button` handler to prevent the user from
// accidentally registering multiple onClicked listeners.
chrome.action.onClicked.addListener(
    (tab) => { chrome.tabs.create({url : 'https://html5zombo.com/'}); });

document.getElementById('onclicked-button')
    .addEventListener('click', async () => {
      // Our listener will only receive the action's click event after clear out
      // the popup URL
      await chrome.action.setPopup({popup : ''});
      await showCurrentPage();
    });

document.getElementById('onclicked-reset-button')
    .addEventListener('click', async () => {
...
document.getElementById('badge-text-input')
    .addEventListener('input', debounce(200, async (event) => {
                        let text = event.target.value;
                        await chrome.action.setBadgeText({text});
                        showBadgeText();
                      }));

document.getElementById('clear-badge-button')
    .addEventListener('click', async () => {
      await chrome.action.setBadgeText({text : ''});
      showBadgeText();
    });
...
async function showBadgeColor() {
  ...
  document.getElementById('current-badge-bg-color').value =
...
document.getElementById('set-badge-background-color-button')
    .addEventListener('click', async () => {
      ...
      //
      // NOTE: Alpha color cannot be set due to crbug.com/1184905. At the time
      // of writing (Chrome 89), an alpha value of 0 sets the default color
      // while a value of 1-255 will make the RGB color fully opaque.
      if (Math.random() < 0.1) {
        color.push(0);
      } else {
        color.push(255);
...
document.getElementById('reset-badge-background-color-button')
    .addEventListener('click', async () => {
...
let lastIconIndex = 0;
document.getElementById('set-icon-button')
    .addEventListener('click', async () => {
      ...
      if (index === lastIconIndex) {
        // Dupe detected! Increment the index & modulo to make sure we don't go
        // out of bounds
      ...
      // There are easier ways for a page to extract an image's imageData, but
      // the approach used here works in both extension pages and service
      // workers.
...
document.getElementById('reset-title-button')
    .addEventListener('click', async () => {
      let manifest = chrome.runtime.getManifest();
