...
/**
 * There's an issue with the chrome.action.setIcon api where it requires the .js file to be in the same folder as the manifest for the "Path" key to properly work if obtaining it with 
 *  manifest.action.default_icon
 * 
 * What happens? The console.log outputs the regular manifest details without any issues, however
 * the actual set Icon shows an error trying to load from /demo/icons/36.png
 * instead of actually handling the current folder
 * 
 * This can be easily resolved within the API to look for the actual path 
 * where the function was executed and compare it to the path of the manifest in case a manifest was piped as a path.
 * 
 * Reported an issue / request https://bugs.chromium.org/p/chromium/issues/detail?id=1354936
 */
document.getElementById('reset-icon-button').addEventListener('click', () => {
  ...
  console.log(manifest.action.default_icon);
});
