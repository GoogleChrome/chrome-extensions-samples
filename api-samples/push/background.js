...
// by subscribeUserVisibleOnlyFalse() and subscribeUserVisibleOnlyTrue();
should_show_notif = true;

// Call this to subscribe the extension to a push notification server with
// `userVisibleOnly: false` to receive silent messages.
function subscribeUserVisibleOnlyFalse() {
  should_show_notif = false;
...
// Call this to subscribe the extension to a push notification server with
// `userVisibleOnly: true` to receive regular push messages requiring a
// notification.
function subscribeUserVisibleOnlyTrue() {
  should_show_notif = true;
  const applicationServerKey = urlB64ToUint8Array(applicationServerPublicKey);
  self.registration.pushManager.subscribe({
    // With our new change, this can be set to false. Before/now it must always
...
function updateSubscriptionOnServer(subscription) {
  ...
  console.log("[Service Worker] Subscription data to be sent to the push" +
    "notification server: ");
...
self.addEventListener('push', function(event) {
  ...
  const user_visible_notification_options = {
    body: 'Yay it works! Received push notifcation to the chrome extension.',
  };
  ...
  if (should_show_notif) {
    const notificationPromise = self.registration.showNotification(
      user_visible_notification_title, user_visible_notification_options);
